...
impl ResponseError for ApplicationError {
    fn status_code(&self) -> StatusCode {
        match self {
            ApplicationError::EParsingError(_)
            | ApplicationError::EAuthenticationError(_)
            | ApplicationError::EAuthorisationError(_) => StatusCode::BAD_REQUEST,

            ApplicationError::EDatabaseError(_)
            | ApplicationError::NotImplementedByConnector(_)
            | ApplicationError::EMetrics(_)
            | ApplicationError::EIo(_)
            | ApplicationError::ConfigurationError(_)
            | ApplicationError::EEncryptionError(_)
            | ApplicationError::EUnexpectedError(_) => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
