...
#[instrument(skip_all)]
pub async fn construct_payment_router_data<'a, F, T, Res>(
    state: &'a AppState,
    payment_data: PaymentData<F>,
    connector_id: &str,
    merchant_account: &storage::MerchantAccount,
) -> RouterResult<(PaymentData<F>, types::RouterData<F, T, Res>)>
where
    T: TryFrom<PaymentData<F>>,
    types::RouterData<F, T, Res>: Feature<F, T, Res>,
    F: Clone,
...
impl From<String> for types::PaymentsResponseData {
    fn from(trans_id: String) -> Self {
        let res = types::PaymentsResponseData {
            resource_id: types::ResponseId::ConnectorTransactionId(trans_id),
...
impl From<String> for types::PaymentsSessionResponseData {
    fn from(trans_id: String) -> Self {
...
#[instrument(skip_all)]
pub async fn construct_payment_session_router_data<'a, F, T, Res>(
    state: &'a AppState,
    payment_data: PaymentData<F>,
    connector_id: &str,
    merchant_account: &storage::MerchantAccount,
    ...
    PaymentData<F>,
    types::RouterData<F, T, types::PaymentsSessionResponseData>,
)>
where
    T: TryFrom<PaymentData<F>>,
    types::RouterData<F, T, types::PaymentsSessionResponseData>: Feature<F, T, Res>,
    F: Clone,
    error_stack::Report<errors::ApiErrorResponse>:
        std::convert::From<<T as TryFrom<PaymentData<F>>>::Error>,
{
