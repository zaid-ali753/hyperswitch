...
use crate::{
    ...
    routes,
    services::{api, build_redirection_form, logger, ApplicationResponse},
    types::storage,
};
    ...
    T: std::fmt::Debug,
{
    ...
    match resp {
        Ok(ApplicationResponse::Json(router_resp)) => {
        ...
        Ok(ApplicationResponse::StatusOk) => api::http_response_ok(),
        Ok(ApplicationResponse::TextPlain(text)) => api::http_response_plaintext(text),
        ...
        Ok(ApplicationResponse::Form(form_data)) => build_redirection_form(&form_data)
            .respond_to(request)
            .map_into_boxed_body(),
        Err(error) => {
